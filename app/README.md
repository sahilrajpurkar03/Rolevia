...
StepStone requests only the public first search page, without query-string pagination, to respect its published robots rules. Requested result limits are upper bounds, not guaranteed counts. Hosted requests may still time out; no PC runner, proxy or undocumented private API is used.

For optional real AI drafting, add `GEMINI_API_KEY` from Google AI Studio to secure server hosting settings and redeploy. Use the free tier without enabling billing. Never paste keys into chat, source files or client-side variables. Missing configuration returns 503 without changing drafts. The authenticated same-origin `/api/letters/generate` route requires explicit consent, bounds job input, uses the account's saved profile, and calls `gemini-2.5-flash` by default. Set the server-only `GEMINI_MODEL` variable to use another supported model; a 503 from a configured custom model retries with the stable default. It limits requests to one per minute and 20 per rolling day per account using private `check_runs` claims; AI claims are excluded from job-check activity.
