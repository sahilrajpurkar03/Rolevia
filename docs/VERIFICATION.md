...
- The owner confirmed replacing the exposed Gemini key and revoking the old key. After repeated HTTP 503 responses from Gemini 3.8 Flash, the route was switched to the supported stable `gemini-2.5-flash` default; a server-only `GEMINI_MODEL` override is supported and falls back to that default on HTTP 503. The real production request then returned HTTP 200 with three paragraphs and two exact evidence quotes verified against the synthetic profile; the test account was deleted. Generation with the replacement key is verified. Revocation was owner-confirmed, not independently inspected.
