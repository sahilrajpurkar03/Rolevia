...
- The owner confirmed replacing the exposed Gemini key and revoking the old key. After the provider retired `gemini-2.5-flash` for new users, the route uses the provider-recommended `gemini-3.6-flash` default; a server-only `GEMINI_MODEL` override is supported and falls back to that default on HTTP 404 or 503. The real production request then returned HTTP 200 with three paragraphs and two exact evidence quotes verified against the synthetic profile; the test account was deleted. Generation with the replacement key is verified. Revocation was owner-confirmed, not independently inspected.
