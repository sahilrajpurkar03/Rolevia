...
1. **High: incomplete operator address.** Updated 12 September 2026: the owner supplied a publishable name, town/postcode/country, and contact email, now shown in the legal pages. The street and house number for a complete address for service are still missing. The imprint and policies remain explicitly incomplete drafts. Noncommercial operation and a beta label do not automatically exempt the service from applicable German/EU obligations. Obtain qualified advice on applicable DDG/MStV identification duties and GDPR transparency obligations; complete the address before broader collection of personal data. Publishing a contact email alone does not resolve this blocker.
2. **High: no retention/deletion operation or verified legal bases.** No automated record retention schedule, self-service deletion, verified request-handling procedure, backup deletion policy, or confirmed provider transfer arrangements exist. Confirm purposes and legal bases, DPAs, regions, international safeguards and retention; implement a workable data-subject request process. A contact email is now published, but inbox monitoring and response procedures have not been independently verified. No policy promises deletion deadlines the app cannot meet.
...
- Every rendered app route inherits a beta warning with an Accept button. A versioned, browser-local acknowledgement dismisses the banner across routes, reloads, and tabs; legal links remain in a footer. Storage failures fall back to in-memory dismissal. This is acknowledgement of the notice, not consent to data processing or legal terms. Update the notice version when revising the warning. The offline fallback retains its static beta notice. This is on-site disclosure, not an email notification to existing account holders or an account-level consent system. Previously installed offline caches may retain an older page until reconnect/update.
