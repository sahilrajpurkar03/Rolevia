...
Daily checks attempt the manual-search sources (the four-source Python worker, StepStone, Arbeitsagentur and both public feeds). This does not resolve upstream Google/StepStone availability. Per-account search time is capped at 200 seconds; incomplete queries are reported. Each invocation attempts at most four unclaimed accounts concurrently. Twenty-five once-daily cron entries drain the pending list, supporting up to 100 opted-in accounts without a PC runner or paid service. The database's unique account/day claim prevents duplicate processing during overlapping invocations; duplicates are skipped without consuming an attempt slot. Invalid profiles receive failed daily claims so they cannot starve later accounts. Failed or interrupted attempts are not automatically retried that day. HTTP 207 reports failures or accounts left for later batches; above 100 opted-in accounts the route returns HTTP 503. Accounts enabled after the last batch wait until the next day. Free function, worker and source quotas still apply; simulated 100-account coverage is not a production load guarantee.
...
Set environment variables for the intended deployment environments. Set `NEXT_PUBLIC_SITE_URL` to the final HTTPS origin before the production build. Do not run production cron against a preview database by accident. `app/vercel.json` registers 25 distinct daily batch URLs, scheduled every half hour from 07:00 through 19:00 UTC. Each entry runs once daily; on Hobby the minute is not guaranteed and the final invocation may start by 19:59 UTC. The `batch` query parameter identifies the cron entry, not an account partition: every invocation drains the same claim-protected pending list. Confirm all 25 entries are enabled in Vercel and inspect execution logs. Vercel currently permits 100 cron entries on Hobby, but function usage quotas still apply. Do not enable paid overages or upgrade billing for this pilot.
